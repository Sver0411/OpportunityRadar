...
（Linux Foundation 下的社区，出现在本轮真实机会名里）当成未完成标记 —— 这是测试被真实数据暴露的缺陷，
已为正名加白名单（**真正的英文待办标记**——那一串大写字母后面跟冒号——仍会被抓到）。
**必须说明：本轮第一次提交（`594703c`）与第二次（`3ddc4ee`）都是在测试失败的状态下推送的**
（我当时没检查输出就提交了；第二次失败是因为我在这段解释里写了字面的待办标记，又触发了同一个扫描器），
修复见最后一次提交。这段说明本身也已改写，避免再触发它。
