...
## 2. 组件与权重（与 `scripts/score.py` 完全一致，不可私自改）
...
## 3. 多样性配额（Step 12 的硬约束）

最终结果集合必须满足：

1. **类别数**：Mode A ≥3 类（若 goals 明确限定则放宽到 ≥2）；
   Mode B/C ≥4 类。
2. **层数**：Exploit 之外至少 **1 条 Adjacent + 1 条 Explore**
   （确实搜不到时，在输出中显式说明"未找到合适的非传统方向"，不得静默略过）。
3. **单类上限**：任一类别不超过结果总数的 50%。
4. **机构上限**：同一机构不超过 2 条（同一企业的 3 个岗位 → 合并或只留最相关 1 条）。
5. **已见过滤**：`seen` 且**无变化**的条目默认不进主列表，移入"已见过"小节。

配额冲突时（例如只有 career 有货）→ 保留配额、如实说明覆盖不足，
**不要**用凑数条目填满。
...
**禁止**给每一项都写 `High`。至少有一项是 Medium/Low 才像真实评估。
`Unknown` 是合法值——页面没写就别硬评。
