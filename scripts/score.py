...
def score_all(profile, opps, seen_index=None, today=None, strict=False, context=None):
    ...
    for opp in opps:
        ...
        results.append({
            ...
            # 把 portfolio / bridge 需要的字段透传出来（否则资源约束算不出小时数）
            "effort": opp.get("effort"),
            "time_commitment": opp.get("time_commitment"),
            "time_to_value": opp.get("time_to_value"),
            "future_optionality": opp.get("future_optionality"),
            "outcomes": opp.get("outcomes") or {},
            "produces": as_list(opp.get("produces")),
            "layer": opp.get("layer"),
